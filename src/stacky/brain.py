...
from .llm import ChatMessage, LMStudioClient, LMStudioError
...
class StackyBrain:
    def __init__(self, soul: StackySoul, memory: MemoryStore, lmstudio: LMStudioClient) -> None:
        self.soul = soul
        self.memory = memory
        self.lmstudio = lmstudio

    async def respond(self, user_text: str) -> BrainReply:
        memories = tuple(_dedupe_memories([*self.memory.pinned(limit=6), *self.memory.recall(user_text, limit=5)]))
        messages = self._messages(user_text, memories)
        remembered: list[Memory] = []
        try:
            response = await self.lmstudio.chat(messages)
        except LMStudioError as exc:
            spoken = (
                "Jeg kan ikke få fat i min lokale LM Studio-hjerne lige nu. "
        ...
        spoken_response = _spoken_response_for_live(user_text, response)
        self.memory.remember(
    ...
    def _messages(self, user_text: str, memories: tuple[Memory, ...]) -> list[ChatMessage]:
...
def _live_answer_rule(user_text: str) -> str:
    if _wants_detail(user_text):
        return "Brugeren beder sandsynligvis om detaljer; giv stadig en kort konklusion først."
    return "Dette er live samtale: hold svaret kort som default, helst under cirka 240 tegn."


def _spoken_response_for_live(user_text: str, response: str) -> str:
    max_chars = 420 if _wants_detail(user_text) else 240
    return compact_for_speech(response, max_chars=max_chars)

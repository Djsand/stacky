...
- Stacky firmware variant `official-0.1.24` has been added as a local patch on top of official firmware.
- Repro patch: `patches/official-stackchan/0001-stacky-bridge.patch`
- Latest body patch version: official StackChan `1.4.1` plus `AppStacky` / bridge `official-0.1.24`.

## Local Flash Command

Use this repo script from `C:\Users\nicol\stackchan`:

```powershell
.\scripts\flash-stackchan-firmware.ps1
```

It uses:

- ESP-IDF: `C:\Users\nicol\esp\esp-idf-v5.5.4`
- Firmware cwd: `C:\Users\nicol\stackchan\vendor\m5stack-stackchan\firmware`
- Short path aliases: `I:` for ESP-IDF and `S:` for firmware
- CoreS3 serial port: `COM3`
...
## Stacky App Variant Official-0.1.24
...
- Boot logo is Stacky-branded: black screen, bitmap ghost generated from `AppIcon-1024.png`, `STACKY` title, and `official-0.1.24` version text instead of the upstream `STACKCHAN` boot label.
